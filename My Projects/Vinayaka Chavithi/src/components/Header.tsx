...
export default function Header(){
  ...
  return (
    <header className={`fixed z-30 w-full transition-shadow ${scrolled ? 'backdrop-blur bg-white/60 shadow' : 'bg-transparent'}`}>
      ...
      {/* mobile drawer */}
      {open && (
        <div className="fixed inset-0 bg-black/40 md:hidden z-40" onClick={()=>setOpen(false)}>
          {/* Fixed close button so it's always visible above the drawer */}
          <button
            className="fixed right-4 top-4 p-2 rounded-full bg-white shadow-md border border-slate-200 text-slate-800 z-60"
